...
import { apiFetch, resolveMediaUrl } from './config';
...
export default function App() {
  ...
  const runTest1 = async () => {
    ...
    try {
      const res = await apiFetch('/api/test-render-1', {
        method: 'POST',
  ...
  const runTest2 = async () => {
    ...
    try {
      const res = await apiFetch('/api/test-render-2', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          localPath: selectedVideo?.localPath || 'public/videos/football_match.mp4',
  ...
  const runTest3 = async () => {
    ...
    try {
      const res = await apiFetch('/api/test-render-3', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          localPath: selectedVideo?.localPath || 'public/videos/football_match.mp4',
  ...
  const runCompleteRealPipeline = async (customStyle?: AIStyle) => {
    ...
    try {
      // 1. Gemini Analysis
      const analyzeRes = await apiFetch('/api/analyze-video', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const renderRes = await apiFetch('/api/render-full-cinematic', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          localPath: selectedVideo.localPath || 'public/videos/football_match.mp4',
      ...
      // 3. Gemini QC Review
      try {
        const qcRes = await apiFetch('/api/qc-review', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleReset = () => {
    if (presets.length > 0) {
      ...
      if (presets[0].posterUrl) setActivePlayerPoster(resolveMediaUrl(presets[0].posterUrl));
